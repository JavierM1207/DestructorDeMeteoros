...
const canvas = document.getElementById('gameCanvas'); const ctx = canvas.getContext('2d');
    ...
    let multishotActive = false; let multishotLevel = 1;
    ...
    const MAX_BG_PARTICLES = 60; const BG_PARTICLE_SPAWN_INTERVAL = 3; let framesSinceLastBgParticle = 0;
...
function playSound(type) {
    ...
    function spawnMeteor() { if (meteors.length < 10 && gameRunning && !paused) { let difficulty = getDifficultyFactor(); if(boostActive) difficulty *=1.5; const minS=dimensions.width*0.03, maxS=dimensions.width*0.08; const size = Math.random()*(maxS-minS)+minS; const x = Math.random()*(dimensions.width-size)+size/2; const y = -size/2; let speedY = (Math.random() * 1.5 + 0.8) * difficulty; let meteorType = 'normal'; if (Math.random() < 0.20) { meteorType = 'metallic'; } meteors.push(new Meteor(x,y,size/2,speedY, meteorType)); } }
    ...
    function updateGame() {
      ...
      if (superPowerActive && --superPowerTimer <=0) { superPowerActive = false; currentOffensivePowerType = null; multishotLevel = 1; multishotActive = false; showMessage("Super Poder Agotado", 2000); }
